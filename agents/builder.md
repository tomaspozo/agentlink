...
- `agentlink env deploy [name]` — Apply schemas + deploy edge functions to the chosen env. Picker if `name` is omitted; preselects the active env.
- `agentlink env deploy prod` — Push to prod (requires y/N confirmation).
- `agentlink env deploy <name> --dry-run` — Preview the target without applying.
- `agentlink env add prod` — Connect / relink a production environment (full bootstrap: schemas + functions + vault + PostgREST + auth).
- `agentlink env add <name> --retry` — Re-apply the full setup if a previous deploy died mid-way or config drifted (schemas, functions, secrets, PostgREST + auth).
- `agentlink env use <name>` — Switch the active environment (local ↔ cloud-dev ↔ prod). `env use prod` requires confirmation.

When users ask about deploying, point them at `agentlink env deploy` (with `--dry-run` if they want a preview). The agent always builds against the active dev environment. Never run `env deploy` or target production directly.
