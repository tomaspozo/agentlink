...
1. **The live database** (local or cloud) — via `pnpm exec agentlink db apply` or `psql`, so changes take effect immediately
...
**Apply methods:**
- **Batch (recommended):** `pnpm exec agentlink db apply` — applies all schema files onto the DB (ALTER-aware, runs in-process — **no Docker**), so changing an existing object (e.g. adding a column) just works. DB URL auto-resolved from `.env.local`.
...
2. Applies via `pnpm exec agentlink db apply` — every file write must be followed by an apply
...
Apply via `pnpm exec agentlink db apply`.
...
**4. Generate types:**
Types are auto-generated by `db apply`. To regenerate manually: `pnpm exec agentlink db types`.
...
**1. Edit the `CREATE TABLE` definition** in `supabase/database/schemas/public/tables/readings.sql` — add the column to the column list — then run `pnpm exec agentlink db apply`. It diffs the changed file against the live DB and emits the `ALTER TABLE … ADD COLUMN` for you; **no hand-written `ALTER` needed.**
```sql
