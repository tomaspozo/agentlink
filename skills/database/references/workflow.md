...
1. **The live database** (local or cloud) — via `npx agentlink-sh@latest db apply` or `psql`, so changes take effect immediately
2. **The schema files** — in `supabase/database/`, so the source of truth stays in sync
...
**Apply methods:**
- **Batch (recommended):** `npx agentlink-sh@latest db apply` — applies all schema files onto the DB (ALTER-aware, runs in-process — **no Docker**), so changing an existing object (e.g. adding a column) just works. DB URL auto-resolved from `.env.local`.
...
2. Applies via `npx agentlink-sh@latest db apply` — every file write must be followed by an apply
...
Works in both local and cloud mode. `db apply` auto-generates types, so this is only needed if you applied SQL manually or want to regenerate without applying.

**Prerequisite:** none beyond the CLI — `db apply` / `db migrate` run in-process with **no Docker**.
...
**1. Create auth functions** — `supabase/database/schemas/public/functions/_auth_reading.sql`:
```sql
...
Apply via `npx agentlink-sh@latest db apply`.

**2. Create the entity file** — `supabase/database/schemas/public/tables/readings.sql`:
```sql
-- Table
...
-- Grant explicitly (default-deny). The api.* RPCs are SECURITY INVOKER, so the
-- caller role needs this; db apply applies it on dev and db migrate carries it
-- into the migration for prod. RLS still gates rows. anon is never granted.
...
**3. Create API functions** — `supabase/database/schemas/api/functions/reading.sql`:
```sql
...
**4. Generate types:**
Types are auto-generated by `db apply`. To regenerate manually: `npx agentlink-sh@latest db types`.
...
**1. Edit the `CREATE TABLE` definition** in `supabase/database/schemas/public/tables/readings.sql` — add the column to the column list — then run `npx agentlink-sh@latest db apply`. It diffs the changed file against the live DB and emits the `ALTER TABLE … ADD COLUMN` for you; **no hand-written `ALTER` needed.**
```sql
-- in the CREATE TABLE column list:
archived_at timestamptz DEFAULT NULL
```

**2. Add index** if needed — add it to the same `readings.sql` file and `db apply` again (it emits the `CREATE INDEX`):
```sql
...
**3. Add the function** — update `supabase/database/schemas/api/functions/reading.sql` and `db apply`:
```sql
...
The trigger function `public.set_updated_at()` is scaffolded by the CLI in `supabase/database/schemas/public/functions/set_updated_at.sql` — the agent doesn't need to create it.

**1. Create trigger** — add it to `supabase/database/schemas/public/tables/readings.sql`, then `db apply`:
```sql
