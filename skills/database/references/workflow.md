...
**Apply methods:**
- **Batch (recommended):** `npx agentlink-sh@latest db apply` — applies all schema files with correct ordering via `pgdelta`. DB URL auto-resolved from `.env.local`.
...
**1. Update the table definition** in `supabase/database/schemas/public/tables/readings.sql` and apply the ALTER via `psql`:
```sql
...
**2. Add index** if needed — update `supabase/database/schemas/public/tables/readings.sql` and apply via `psql`:
```sql
...
**3. Add the function** — update `supabase/database/schemas/api/functions/reading.sql` and apply via `psql`:
```sql
...
**1. Create trigger** — `supabase/database/schemas/public/tables/readings.sql`. Apply via `psql`:
```sql
