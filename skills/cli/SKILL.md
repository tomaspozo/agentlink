...
Schema files are **one object per file** (`public/tables/<table>.sql`, `public/functions/<fn>.sql`, `api/functions/<rpc>.sql`). Order doesn't matter: pg-delta topologically sorts statements by dependency at apply time, so a single recursive glob safely picks up every file regardless of count or naming.
