...
- **Theming:** `src/styles.css` is pure, unmodified shadcn output (CSS variables
  for colors, radius, font) — edit it directly to reskin the whole app, or run
  `shadcn init --preset <code> --force --no-reinstall` to swap themes wholesale.
  No custom tokens or classes live here — see the frontend skill's
  [Theming & customization](../../frontend/SKILL.md#theming--customization).
